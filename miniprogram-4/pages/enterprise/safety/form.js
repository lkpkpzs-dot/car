...
Page({
  ...
  validate() {
    ...
    if (!form.idCardNo.trim()) {
      wx.showToast({ title: '身份证号不能为空', icon: 'none', duration: 2000 });
      return false;
    }
    if (!age || age < 21 || age > 50) {
      wx.showToast({ title: '年龄必须为21-50', icon: 'none', duration: 2000 });
      return false;
    }
    if (!form.licenseType) {
      wx.showToast({ title: '驾驶证类型不能为空', icon: 'none', duration: 2000 });
      return false;
    }
    if (!form.firstLicenseDate) {
      wx.showToast({ title: '初次领证日期不能为空', icon: 'none', duration: 2000 });
      return false;
    }
    // 验证驾龄必须超过3年
    const firstLicenseDate = new Date(form.firstLicenseDate);
    const now = new Date();
    const yearsDiff = (now - firstLicenseDate) / (1000 * 60 * 60 * 24 * 365);
    if (yearsDiff < 3) {
      wx.showToast({ title: '驾龄必须超过3年', icon: 'none', duration: 2000 });
      return false;
    }
    const missing = materialFields.find(item => !form[item.key]);
    if (missing) {
      wx.showToast({ title: `请上传${missing.label}`, icon: 'none', duration: 2000 });
      return false;
    }
    return true;
  },
