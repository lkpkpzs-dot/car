...
Page({
  ...
  async loadPendingCounts() {
    try {
      ...
      // 获取未读消息数量（轻量接口，替代全量 myMessages）
      let unreadCount = 0;
      try {
        const unreadRes = await request.get('/sysMessage/unreadCount');
        if (unreadRes.code === 200 && unreadRes.data !== undefined && unreadRes.data !== null) {
          unreadCount = Number(unreadRes.data) || 0;
        }
      } catch (msgErr) {
        console.error('[Load Unread Count] Failed:', msgErr);
      }
