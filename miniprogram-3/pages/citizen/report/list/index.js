...
Page({
  ...
  onLoad(options) {
    ...
    const tab = statusTabMap[options.status] !== undefined ? statusTabMap[options.status] : 0;
  ...
  async loadList() {
    ...
    try {
      const res = await request.get('/citizenReport/citizenList', {});
  ...
  filterList() {
    ...
    if (currentTab === 0) {
      ...
      list = validList.filter(item => item.processStatus === 0);
    } else if (currentTab === 2) {
      list = validList.filter(item => item.processStatus === 1 || item.processStatus === 4);
    } else if (currentTab === 3) {
      list = validList.filter(item => item.processStatus === 2);
    } else if (currentTab === 4) {
      list = validList.filter(item => item.processStatus === 3);
    }
