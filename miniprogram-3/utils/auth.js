  ...
  return new Promise((resolve, reject) => {
    wx.login({
      success: function(res) {
        if (res.code) {
          wx.request({
            url: 'https://wbd726ad.natappfree.cc/auth/login',
            method: 'POST',
            timeout: 10000,
            data: { code: res.code },
            success: function(loginRes) {
              ...
              if (loginRes.data && loginRes.data.data) {
                var data = loginRes.data.data;
                var token = data.token;
                var refreshToken = data.refreshToken;
                var user = data.user;
                var enterpriseName = data.enterpriseName;
                var qualificationStatus = data.qualificationStatus;
                ...
                // 2. 整合用户信息并保存
                var enterpriseUtil = require('./enterprise.js');
                var fullUserInfo = {
                  userId: user.userId,
                  openid: user.openid,
                  ...
                  phone: user.phone,
                  authEnterpriseId: user.authEnterpriseId,
                ...
                // 3. 处理角色跳转
                var rawRoleType = user ? user.roleType : 3;
                // 如果用户有 authEnterpriseId，优先视为企业用户
                if (user && user.authEnterpriseId && rawRoleType !== 2) {
                  rawRoleType = 2;
                }
                var mappedRole = mapRoleType(rawRoleType);
                setRole(mappedRole);
